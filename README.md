...
>
> **Second, separate blocker found later:** Bedrock Mantle itself (the stand-in) started intermittently rejecting **multi-turn** tool-calling conversations specifically — single-turn calls still succeed — surfaced by this session's testing volume against the deployed AgentCore runtimes. Two plausible fixes were tried (explicit client timeouts; a Mantle-multi-turn compat model shipped in AWS's own AgentCore CLI scaffold) and neither resolved it. Documented honestly in [agentcore-deploy/README.md](agentcore-deploy/README.md) rather than claimed-fixed; the underlying architecture was genuinely verified working before this appeared.
>
