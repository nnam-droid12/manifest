...
| Orchestrator Agent | Coordinates the swarm per active load; maintains state in AgentCore Memory; decides autonomous vs. human-review paths | working, **deployed live to Bedrock AgentCore Runtime with real AgentCore Memory** — chains Load-Matching → Carrier Vetting → [gate] → Rate Intelligence → Carrier Outreach locally; the gate is enforced in code, not left to a model's judgment. Verified live on two carriers — both correctly escalated to human review. A load's history now genuinely persists across separate cloud invocations via AgentCore Memory (not a local cache) — verified with two independent `invoke-agent-runtime` calls where the second recalled a prior carrier finding verbatim without re-running any tools; getting this right took finding and fixing a real bug (SSE stream consumption meant the save call never ran) — see [agentcore-deploy/README.md](agentcore-deploy/README.md) for the full story |
